...
def f_import(dfs, scn, path, start_date='2030-01-01_00:00'):
    ...
    # try:
    # df_m_input_commodity = pandas.read_excel(
    #     './input/om-threenode-storage-transmission.xlsx',
    ...
    def f_get_template_primary_energy(df_m_input_commodity):
        ...
        if 'max' not in df_m_input_commodity:
            ...
            if (
                math.isnan(df_m_input_commodity['max'])
                or df_m_input_commodity['max'] == math.inf
            ):
                df_m_input_commodity['max'] = 1000000
        ...
        if (
            df_m_input_commodity['Commodity'] == 'Solar'
            or df_m_input_commodity['Commodity'] == 'Wind'
            or df_m_input_commodity['Commodity'] == 'Run of River'
        ):
            return [
                [
                    '#code',
                    df_m_input_commodity['Commodity'].replace(" ", "_")
                    + '_'
                    + df_m_input_commodity['Site'].replace(" ", "_"),
                    '#name',
                    df_m_input_commodity['Commodity'].replace(" ", "_")
                    + '_'
                    + df_m_input_commodity['Site'].replace(" ", "_"),
            ...
            return [
                ...
                [
                    ...
                    '#data',
                    str(start_date),
                    str(df_m_input_commodity['price'] * 1000),
                ],
                ['#endtable'],
                ['#endblock'],
            ]
    ...
    with open(path + '/PrimaryEnergy.csv', 'w') as writeFile:
        ...
        for index, row in df_m_input_commodity.iterrows():
            ...
            if df_m_input_commodity['Commodity'] != 'Elec':
                writer.writerows(
                    f_get_template_primary_energy(df_m_input_commodity)
                )
