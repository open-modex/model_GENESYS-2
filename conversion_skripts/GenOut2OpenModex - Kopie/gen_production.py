...
def get_production(file_to_read):
    ...
    for ctr_converters in range(len(all_gen_elements)):
        ...
        '''
        if conv_start_converted == init_start_converted:
            temp_time_shift = 1
        else:
            diffTime = abs((parse(conv_start_converted) - parse(init_start_converted)).days)
            temp_time_shift = 0
            if diffTime > 1:
                init_start_converted = conv_start_converted
        ...
        '''
        data_length = len(float_conv_data)
        if temp_time_shift:
            float_conv_data = float_conv_data[0:data_length-1]
            time_series_tmp = time_series_tmp[0:(data_length-1)]
        else:
            float_conv_data.insert(0, 0)
            float_conv_data = float_conv_data[0:data_length - 2]
            time_series_tmp = time_series_tmp[0:(data_length - 2)]
        '''
        data_length = len(float_conv_data)
        float_conv_data = float_conv_data[0:data_length - 1]
        time_series_tmp = time_series_tmp[0:(data_length - 1)]
    ...
    #nans auffuellen mit Nullen
    full_dataframe = full_dataframe.fillna(0)

    # Daten in das CSV-File schreiben-----------------------------------------------------------------------------------
    with open(save_path, 'w', newline='') as csvfile:
        filewriter = csv.writer(csvfile, delimiter=',', quoting=csv.QUOTE_MINIMAL)
        header_row = ",".join(str(header_str) for header_str in list(full_dataframe))
        filewriter.writerow(['Production per timestep in MWh, '+header_row])
        for ctr_data_points in full_dataframe.iterrows():
            ...
            rowData = ctr_data_points[1]._ndarray_values
            data_point = ''
            for anz_col in rowData:
                data_point = data_point + str(format(anz_col,'.2f')) + ','
